...
final_filtered_trajectories <- readRDS("/Users/reneechabot/Desktop/4final_traj.RData")
...
cruise_info <- read.delim(
  '/Users/reneechabot/Library/CloudStorage/GoogleDrive-renee.chabot@stonybrook.edu/.shortcut-targets-by-id/1GSVxGJlWo-R0hbtHEXmwYdG8xHXiGhzo/Shepson Group Drive/Renée/Research/R V Seawolf- Cruises/Cruises/Cruise 4 (4-5-22 to 4-16-22)/moving_data.txt'
  ,
  sep = ",",
  dec = "."
)
  ...
  if (any(final_cruise_info$Day == current_date)) {
    ...
    q <- qplot(x, y) +
     ...
     #scale_x_continuous(breaks=temp_data$Time_local)+
      ...
      labs(
        title = paste("Cruise #4: Measured CH4 for", temp_data$Day, "(5 minute average)"), #"(3 hour average)",
        x = "Time (local)",
        y = "CO2 (ppm)"
      )
